...
public class ElasticsearchResultSet implements ResultSet{
	...
	public ElasticsearchResultSet(Response response) {
	    try {
			JSONObject jobject = new JSONObject(response.body().string());
			this.columns = ((JSONArray)jobject.get("cols")).toList();
			
			
			this.fieldMap = getFieldMap(this.columns);
			
			List<List<Object>> rows = new ArrayList<List<Object>>();
			for(Object e : (JSONArray)jobject.get("rows")) {
				rows.add(((JSONArray)e).toList());
			}
    ...
    private static Map<String, Integer> getFieldMap(List<Object> columns)
    {
        Map<String, Integer> map = new HashMap<>();
        for (int i = 0; i < columns.size(); i++) {
            String name = ((String) columns.get(i)).toLowerCase(ENGLISH);
	...
	public ResultSetMetaData getMetaData() throws SQLException {
		// TODO Auto-generated method stub
		return new ElasticsearchResultSetMetaData(this.columns);
	}
	...
	public String getString(String columnLabel) throws SQLException {
		...
		Object obj = column(columnIndex(columnLabel));
		if (obj instanceof String) {
			return (String)(obj);
		}else {
			return String.valueOf(obj);
